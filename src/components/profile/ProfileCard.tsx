...
function BottomNav({ profile }: { profile: PublicProfile }) {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-30 mx-auto grid max-w-5xl border-t border-black/5 bg-white/95 px-2 py-2 text-center text-xs font-semibold text-aodi-violet-950 shadow-[0_-14px_34px_rgba(24,18,10,0.12)] backdrop-blur md:absolute" style={{ gridTemplateColumns: `repeat(${items.length}, minmax(0, 1fr))` }}>
...
export function ProfileCard({ profile }: { profile: PublicProfile }) {
  ...
  return (
    <article className={`relative mx-auto min-h-dvh w-full max-w-5xl overflow-hidden ${visual.surface} pb-24 shadow-card md:min-h-0 md:rounded-[1.5rem]`}>
      <header className={`relative min-h-[760px] overflow-hidden bg-gradient-to-br ${visual.gradient} text-white md:min-h-[690px]`}>
        ...
        <div className="relative z-10 flex min-h-[760px] flex-col px-5 pb-7 pt-5 sm:px-9 md:min-h-[690px]">
          ...
          <div className="grid flex-1 gap-6 pt-9 md:grid-cols-[0.92fr_1.08fr] md:items-end md:pt-0">
            <div className="relative z-10 pb-3 md:pb-12">
              ...
              <h1 className="mt-4 max-w-[620px] break-words font-display text-[3.8rem] font-bold leading-[0.82] text-white sm:text-[5rem] md:text-[5.8rem]">{profile.displayName}</h1>
